const mainHand = [
  "Expert's Spirithunter",
  "Master's Spirithunter",
  "Grandmaster's Spirithunter",
  "Elder's Spirithunter",

  "Expert's Siegebow",
  "Master's Siegebow",
  "Grandmaster's Siegebow",
  "Elder's Siegebow",

  "Expert's Mistpiercer",
  "Master's Mistpiercer",
  "Grandmaster's Mistpiercer",
  "Elder's Mistpiercer",

  "Expert's Brimstone Staff",
  "Master's Brimstone Staff",
  "Grandmaster's Brimstone Staff",
  "Elder's Brimstone Staff",

  "Expert's Dawnsong",
  "Master's Dawnsong",
  "Grandmaster's Dawnsong",
  "Elder's Dawnsong",

  "Expert's Greataxe",
  "Master's Greataxe",
  "Grandmaster's Greataxe",
  'The Hand of Khor',

  "Expert's Bridled Fury",
  "Master's Bridled Fury",
  "Grandmaster's Bridled Fury",
  "Elder's Bridled Fury",

  "Expert's Realmbreaker",
  "Master's Realmbreaker",
  "Grandmaster's Realmbreaker",
  "Elder's Realmbreaker",

  "Expert's Damnation Staff",
  "Master's Damnation Staff",
  "Grandmaster's Damnation Staff",
  "Elder's Damnation Staff",

  "Expert's Permafrost Prism",
  "Master's Permafrost Prism",
  "Grandmaster's Permafrost Prism",
  "Elder's Permafrost Prism",

  "Expert's Energy Shaper",
  "Master's Energy Shaper",
  "Grandmaster's Energy Shaper",
  "Elder's Energy Shaper",

  "Expert's Spiked Gauntlets",
  "Master's Spiked Gauntlets",
  "Grandmaster's Spiked Gauntlets",
  "Elder's Spiked Gauntlets",

  "Expert's Ursine Maulers",
  "Master's Ursine Maulers",
  "Grandmaster's Ursine Maulers",
  "Elder's Ursine Maulers",

  "Expert's Ravenstrike Cestus",
  "Master's Ravenstrike Cestus",
  "Grandmaster's Ravenstrike Cestus",
  "Elder's Ravenstrike Cestus",

  "Expert's Glacial Staff",
  "Master's Glacial Staff",
  "Grandmaster's Glacial Staff",
  "Elder's Glacial Staff",

  "Expert's Grailseeker",
  "Master's Grailseeker",
  "Grandmaster's Grailseeker",
  "Elder's Grailseeker",

  "Expert's Icicle Staff",
  "Master's Icicle Staff",
  "Grandmaster's Icicle Staff",
  "Elder's Icicle Staff",

  "Expert's Galatine Pair",
  "Master's Galatine Pair",
  "Grandmaster's Galatine Pair",
  "Elder's Galatine Pair",

  "Expert's Wailing Bow",
  "Master's Wailing Bow",
  "Grandmaster's Wailing Bow",
  "Elder's Wailing Bow",

  "Expert's Longbow",
  "Master's Longbow",
  "Grandmaster's Longbow",
  "Elder's Longbow",

  "Expert's Grovekeeper",
  "Master's Grovekeeper",
  "Grandmaster's Grovekeeper",
  "Elder's Grovekeeper",

  "Expert's Soulscythe",
  "Master's Soulscythe",
  "Grandmaster's Soulscythe",
  "Elder's Soulscythe",

  "Expert's Camlann Mace",
  "Master's Camlann Mace",
  "Grandmaster's Camlann Mace",
  "Elder's Camlann Mace",

  "Expert's Hand of Justice",
  "Master's Hand of Justice",
  "Grandmaster's Hand of Justice",
  "Elder's Hand of Justice",

  "Expert's Malevolent Locus",
  "Master's Malevolent Locus",
  "Grandmaster's Malevolent Locus",
  "Elder's Malevolent Locus",

  "Expert's Enigmatic Staff",
  "Master's Enigmatic Staff",
  "Grandmaster's Enigmatic Staff",
  "Elder's Enigmatic Staff",

  "Expert's Occult Staff",
  "Master's Occult Staff",
  "Grandmaster's Occult Staff",
  "Elder's Occult Staff",

  "Expert's Arcane Staff",
  "Master's Arcane Staff",
  "Grandmaster's Arcane Staff",
  "Elder's Arcane Staff",

  "Expert's Lifecurse Staff",
  "Master's Lifecurse Staff",
  "Grandmaster's Lifecurse Staff",
  "Elder's Lifecurse Staff",

  "Expert's Fallen Staff",
  "Master's Fallen Staff",
  "Grandmaster's Fallen Staff",
  "Elder's Fallen Staff",

  "Expert's Wild Staff",
  "Master's Wild Staff",
  "Grandmaster's Wild Staff",
  "Elder's Wild Staff",

  "Expert's Blight Staff",
  "Master's Blight Staff",
  "Grandmaster's Blight Staff",
  "Elder's Blight Staff",

  "Expert's Bloodletter",
  "Master's Bloodletter",
  "Grandmaster's Bloodletter",
  "Elder's Bloodletter",
];

const headArmor = [
  "Expert's Assassin Hood",
  "Master's Assassin Hood",
  "Grandmaster's Assassin Hood",
  "Elder's Assassin Hood",

  "Expert's Stalker Hood",
  "Master's Stalker Hood",
  "Grandmaster's Stalker Hood",
  "Elder's Stalker Hood",

  "Expert's Royal Hood",
  "Master's Royal Hood",
  "Grandmaster's Royal Hood",
  "Elder's Royal Hood",

  "Expert's Mercenary Hood",
  "Master's Mercenary Hood",
  "Grandmaster's Mercenary Hood",
  "Elder's Mercenary Hood",

  "Expert's Knight Helmet",
  "Master's Knight Helmet",
  "Grandmaster's Knight Helmet",
  "Elder's Knight Helmet",

  "Expert's Judicator Helmet",
  "Master's Judicator Helmet",
  "Grandmaster's Judicator Helmet",
  "Elder's Judicator Helmet",
];

const chestArmor = [
  "Expert's Cleric Robe",
  "Master's Cleric Robe",
  "Grandmaster's Cleric Robe",
  "Elder's Cleric Robe",

  "Expert's Scholar Robe",
  "Master's Scholar Robe",
  "Grandmaster's Scholar Robe",
  "Elder's Scholar Robe",

  "Expert's Robe of Purity",
  "Master's Robe of Purity",
  "Grandmaster's Robe of Purity",
  "Elder's Robe of Purity",

  "Expert's Hellion Jacket",
  "Master's Hellion Jacket",
  "Grandmaster's Hellion Jacket",
  "Elder's Hellion Jacket",

  "Expert's Stalker Jacket",
  "Master's Stalker Jacket",
  "Grandmaster's Stalker Jacket",
  "Elder's Stalker Jacket",

  "Expert's Armor of Valor",
  "Master's Armor of Valor",
  "Grandmaster's Armor of Valor",
  "Elder's Armor of Valor",

  "Expert's Demon Armor",
  "Master's Demon Armor",
  "Grandmaster's Demon Armor",
  "Elder's Demon Armor",

  "Expert's Guardian Armor",
  "Master's Guardian Armor",
  "Grandmaster's Guardian Armor",
  "Elder's Guardian Armor",

  "Expert's Judicator Armor",
  "Master's Judicator Armor",
  "Grandmaster's Judicator Armor",
  "Elder's Judicator Armor",

  "Expert's Knight Armor",
  "Master's Knight Armor",
  "Grandmaster's Knight Armor",
  "Elder's Knight Armor",

  "Expert's Soldier Armor",
  "Master's Soldier Armor",
  "Grandmaster's Soldier Armor",
  "Elder's Soldier Armor",
];

const shoeArmor = [
  "Expert's Royal Sandals",
  "Master's Royal Sandals",
  "Grandmaster's Royal Sandals",
  "Elder's Royal Sandals",

  "Expert's Scholar Sandals",
  "Master's Scholar Sandals",
  "Grandmaster's Scholar Sandals",
  "Elder's Scholar Sandals",

  "Expert's Hunter Shoes",
  "Master's Hunter Shoes",
  "Grandmaster's Hunter Shoes",
  "Elder's Hunter Shoes",

  "Expert's Graveguard Boots",
  "Master's Graveguard Boots",
  "Grandmaster's Graveguard Boots",
  "Elder's Graveguard Boots",
];

const generateOptions = (armorArr, type) => {
  const select = document.createElement('select');
  select.setAttribute('name', `${type}`);
  select.setAttribute('id', `${type}`);

  for (const armor of armorArr) {
    const option = document.createElement('option');
    option.value = armor;
    option.innerHTML = armor;
    select.appendChild(option);
  }

  const label = document.querySelector(`.${type}`);
  label.insertAdjacentElement('afterend', select);
};

generateOptions(mainHand, 'main-hand');
generateOptions(headArmor, 'head-armor');
generateOptions(chestArmor, 'chest-armor');
generateOptions(shoeArmor, 'shoe-armor');

const handleSubmit = (event) => {
  window.alert('Thank you for submitting a regear request!');
};

const buttons = document.getElementsByClassName('submit-button');
for (const button of buttons) {
  // event listener to satisfy project requirements
  button.addEventListener(
    'mouseenter',
    function (event) {
      event.target.style.backgroundColor = '#17628a';
      event.target.style.color = 'white';

      // reset the properties after a short delay
      setTimeout(function () {
        event.target.style.backgroundColor = '';
        event.target.style.color = '';
      }, 500);
    },
    false
  );
}
